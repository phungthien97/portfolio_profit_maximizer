  ...
  const handleExportPDF = async () => {
    ...
    if (metrics) {
      ...
      Object.entries(metrics).forEach(([symbol, metric]) => {
        ...
        doc.text(metric.minPrice !== null ? metric.minPrice.toFixed(2) : 'N/A', 150, yPos);
        doc.text(metric.maxPrice !== null ? metric.maxPrice.toFixed(2) : 'N/A', 180, yPos);
        yPos += 7;
      });
      yPos += 8;
    }
    ...
    doc.setFontSize(12);
    doc.setFont(undefined, 'normal');
    doc.text(`Investment Amount: ${currency} ${parseInt(investmentAmount).toLocaleString()}`, 20, yPos);
    yPos += 7;
    ...
    Object.entries(allocation.allocations).forEach(([symbol, alloc]) => {
      ...
      doc.text(`${alloc.amount.toFixed(2)}`, 140, yPos);
      yPos += 7;
    });
    yPos += 10;
    ...
    if (projectionData && projectionData.portfolioValues.length > 0) {
      ...
      yPos += 7;
      doc.text(`Initial Investment: ${currency} ${parseFloat(investmentAmount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, yPos);
      yPos += 7;
      doc.text(`Projected Value (${projectionYears} years): ${currency} ${finalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, yPos);
      yPos += 7;
      ...
      doc.text(`95% Confidence Interval: ${currency} ${lower.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} - ${currency} ${upper.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, yPos);
      yPos += 12;
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <p className="text-lg font-semibold text-blue-900">
                  {currency} {parseFloat(investmentAmount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
              <div className="p-4 bg-green-50 border border-green-200 rounded-md">
                <p className="text-sm text-green-700 font-medium">Projected Value ({projectionYears} years)</p>
                <p className="text-lg font-semibold text-green-900">
                  {currency} {projectionData.portfolioValues[projectionData.portfolioValues.length - 1].toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
